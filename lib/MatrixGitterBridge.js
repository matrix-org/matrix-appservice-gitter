    ...
    var bridge = new Bridge({
        ...
        controller: {
            ...
            onEvent: function(req, context) {
                ...
                var bridgedRoom = self.getBridgedRoomFromMemory(event.room_id);
                if (bridgedRoom) {
...
MatrixGitterBridge.prototype.getBridgedRoomFromMemory = function(roomId) {
...
MatrixGitterBridge.prototype.getLinkByMatrixId = function(matrixId) {
    var store = this.getRoomStore();

    var room = this.getBridgedRoomFromMemory(matrixId);
    ...
    return store.getEntriesByMatrixId(matrixId).then((remoteLinks) => {
        ...
        if (remoteLinks.length > 1) {
            console.log('WARNING: More than one links mapped to one matrix-id');
        }

        return {
            matrix_room_id : remoteLinks[0].matrix.roomId,
            remote_room_name: remoteLinks[0].remote.roomId
        };
    });
};
