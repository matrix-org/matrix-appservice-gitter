...
function MatrixGitterBridge(config) {
    ...
    if (config.statsd) {
        this._statsd = new StatsD(config.statsd);

        this._sendProcessStats();

        var interval_msec = (config.statsd.interval || 60) * 1000;
        setInterval(this._sendProcessStats.bind(this), interval_msec);
    }
}
...
MatrixGitterBridge.prototype._sendProcessStats = function() {
    ...
    Object.keys(usage).forEach((key) => {
        this._statsd.gauge("process.mem." + key, usage[key]);
    });

    // TODO: process.cpuUsage() is only available at Node 6.2.0
    var cpuusage = getrusage.cpuUsage();

    this._statsd.gauge("process.cpu.user", cpuusage.user);
    this._statsd.gauge("process.cpu.system", cpuusage.system);

    this._statsd.gauge("remote.rooms",
        Object.keys(this._bridgedRoomsByGitterRoomname).length);
    this._statsd.gauge("matrix.rooms",
        Object.keys(this._bridgedRoomsByMatrixId).length);

    this._statsd.gauge("remote.useres",
        Object.keys(this._gitterUserPromisesById).length);

    // TODO: Don't have a counter for this yet
    this._statsd.gauge("matrix.users", 0);
};

MatrixGitterBridge.prototype.incStat = function(name) {
    if (!this._statsd) return;
