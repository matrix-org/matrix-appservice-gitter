...
MatrixGitterBridge.prototype.linkGitterToMatrix = function(gitterRoomName, matrixRoomId) {
    return this.getOrCreateBridgedRoom(gitterRoomName).then((bridgedRoom) => {
        bridgedRoom.linkMatrixRoom(matrixRoomId);
        this._bridgedRoomsByMatrixId[matrixRoomId] = bridgedRoom;
        return bridgedRoom;
    });
};
...
MatrixGitterBridge.prototype.run = function(port) {
    ...
    bridge.loadDatabases().then(() => {
        ...
        entries.forEach((entry) => {
            ...
            return this.linkGitterToMatrix(remote_id, matrix_id).then(() => {
...
MatrixGitterBridge.prototype.actionLink = function(matrixId, gitterName) {
    ...
    return store.getEntriesByMatrixId(matrixId).then((remoteLinks) => {
        ...
        return store.insert({
            ...
            return this.linkGitterToMatrix(gitterName, matrixId);
        }).then(() => {
