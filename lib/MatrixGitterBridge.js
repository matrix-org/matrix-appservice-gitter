...
MatrixGitterBridge.prototype.onUserMappingQuery = function(protocol, fields) {
    if (protocol != "gitter") {
        return Promise.reject({code: 404, err: "Unknown 3PN protocol " + protocol});
    }
    ...
    // TODO(paul): reverse MXID->gitter lookup
        ...
        return Promise.reject({code: 400, err: "Require a 'usernanme' parameter'"});
    }
...
MatrixGitterBridge.prototype.run = function(port) {
    ...
    app.get("/_matrix/app/:ver/thirdparty/user/:protocol", (req, res) => {
        ...
        this.onUserMappingQuery(req.params.protocol, req.query).then(
