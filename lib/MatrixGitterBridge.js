...
MatrixGitterBridge.prototype.onUserMappingQuery = function(req, res) {
    ...
    if ("username" in query) {
        ...
        // Reverse MXID->gitter name lookup
        var username = this.gitterNameFromMxid(query.userid);
        ...
        if (username != null) {
            ...
            res.status(404).json({error: "The requested userid is not recognised by this Application Service"});
            return;
        }
    }
    else {
        res.status(400).json({error: "Require either 'username' or 'userid' parameter"});
        return;
    }
...
MatrixGitterBridge.prototype.run = function(port) {
    ...
    app.get("/_matrix/app/:ver/user", (req, res) => {
        if (req.params.ver != "r0" && req.params.ver != "unstable") {
...
MatrixGitterBridge.prototype.actionLink = function(matrixId, gitterName) {
    ...
    return Promise.all([
        ...
        if (remoteLinks.length) {
            ...
            return Promise.reject("gitter-name " + gitterName + " is already linked to " + matrixLinks[0].matrix);
        }
