...
MatrixGitterBridge.prototype.initialiseMetrics = function() {
...
MatrixGitterBridge.prototype.incCounter = function(name, labels) {
    if (!this._metrics) return;
    this._metrics.incCounter(name, labels);
};

MatrixGitterBridge.prototype.incMatrixCallCounter = function(type) {
    if (!this._metrics) return;
    this._metrics.incCounter("api_calls", {side: "matrix", type: type});
};

MatrixGitterBridge.prototype.incRemoteCallCounter = function(type) {
    if (!this._metrics) return;
...
//   URL.
MatrixGitterBridge.prototype.getUrlForMxc = function(mxc_url) {
...
MatrixGitterBridge.prototype.mangleName = function(name, _default) {
...
MatrixGitterBridge.prototype.getOrCreateGitterUser = function(user) {
    var id = user.id;
    ...
    return this.getRemoteUser(id).then((model) => {
        ...
        u = this._gitterUsersById[id] = new GitterUser(this, model);
        return Promise.resolve(u);
    });
};

// Obtain a GitterUser instance from a Gitter user ID string if one had
//   previously been created (see getOrCreateGitterUser), or null if one does
//   not already exist.
MatrixGitterBridge.prototype.getGitterUserById = function(id) {
    ...
    return this.getRemoteUser(id).then((model) => {
        ...
        u = new GitterUser(this, model);
        this._gitterUsersById[id] = u;
        return Promise.resolve(u);
    });
};

MatrixGitterBridge.prototype.getOrCreateMatrixUser = function(id) {
...
// it
MatrixGitterBridge.prototype.getState = function(roomId, eventType) {
...
MatrixGitterBridge.prototype.listAllUsers = function(matrixId) {
...
MatrixGitterBridge.prototype.listGhostUsers = function(matrixId) {
    return this.listAllUsers(matrixId).then((user_ids) => {
        return user_ids.filter((id) => this.gitterNameFromMxid(id));
    });
};

MatrixGitterBridge.prototype.drainAndLeaveMatrixRoom = function(matrixRoomId) {
...
MatrixGitterBridge.prototype.makeMatrixRoom = function(opts) {
    this.incCounter("api_calls", {side: "matrix", type: "createRoom"});
...
MatrixGitterBridge.prototype.checkLinkPermission = function(matrix_room_id, user_id) {
...
Provisioning.commands.link = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrixId, gitterName, userId) {
        return bridge.checkLinkPermission(matrixId, userId).then((allowed) => {
            if (!allowed) return Promise.reject({
                code: 403,
                text: userId + " is not allowed to provision links in " + matrixId,
            });

            return bridge.actionLink(matrixId, gitterName);
        }).then(
            () => { res.json({}) }
        );
    },
...
Provisioning.commands.unlink = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrixId, gitterName, userId) {
        return bridge.checkLinkPermission(matrixId, userId).then((allowed) => {
            ...
            return bridge.actionUnlink(matrixId, gitterName);
        }).then(
            () => { res.json({}) }
        );
    },
