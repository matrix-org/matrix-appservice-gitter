...
function MatrixGitterBridge(config) {
    ...
    if (config.enable_metrics) {
        ...
        getrusage = require("qrusage");

        var client = this._prom_client = new Prometheus();

        var newGauge = function(name, help) {
            return client.newGauge({
                namespace: "bridge",
                name: name,
                help: help,
            });
        ...
        var newCounter = function(name, help) {
            return client.newCounter({
                namespace: "bridge",
                name: name,
                help: help,
            });
        ...
        this._metrics = {
            // General process-wide
            process_mem: newGauge("process_mem", "memory usage in bytes"),
            process_cpu: newGauge("process_cpu", "CPU usage in microseconds"),

            // Specific to bridging
            rooms: newGauge("rooms", "current count of mapped rooms"),
            users: newGauge("users", "current count of mapped users"),

            received_messages: newCounter("received_messages", "count of received messages"),
            sent_messages: newCounter("sent_messages", "count of sent messages"),
        };

        this._sendProcessStats();
        setInterval(this._sendProcessStats.bind(this), 60 * 1000);
    }

}

MatrixGitterBridge.prototype._sendProcessStats = function() {
    var usage = process.memoryUsage();
    Object.keys(usage).forEach((key) => {
        this._metrics.process_mem.set({type: key}, usage[key]);
    });

    // TODO: process.cpuUsage() is only available at Node 6.2.0
    var cpuusage = getrusage.cpuUsage();

    this._metrics.process_cpu.set({type: "user"}, cpuusage.user);
    this._metrics.process_cpu.set({type: "system"}, cpuusage.system);

    this._metrics.rooms.set({side: "remote"},
        Object.keys(this._bridgedRoomsByGitterRoomname).length);
    this._metrics.rooms.set({side: "matrix"},
        Object.keys(this._bridgedRoomsByMatrixId).length);

    this._metrics.users.set({side: "remote"},
        Object.keys(this._gitterUserPromisesById).length);

    // TODO: Don't have a counter for this yet
    this._metrics.users.set({side: "matrix"}, 0);
};

MatrixGitterBridge.prototype.incMetric = function(name, labels) {
    ...
    this._metrics[name].increment(labels);
};
...
MatrixGitterBridge.prototype.getBridgedRoomByMatrixId = function(roomId) {
...
MatrixGitterBridge.prototype.run = function(port) {
    ...
    bridge.loadDatabases().then(() => {
        ...
        // startup
        if (this._metrics) this._sendProcessStats();
    });

    bridge.run(port, this._config);
    Provisioning.addAppServicePath(bridge, this);

    if (this._prom_client) {
        this._bridge.addAppServicePath({
            method: "GET",
            path: "/metrics",
            handler: this._prom_client.metricsFunc(),
        });
    }
};
...
Provisioning.commands.getlink = new Provisioning.Command({
    ...
    func: function(bridge, req, res, matrixId) {
        var room = bridge.getBridgedRoomByMatrixId(matrixId);
        if (!room) {
