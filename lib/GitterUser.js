...
GitterUser.prototype.updatePresence = function(present) {
  if (present == this._ispresent)
    return;
  this._ispresent = present;

  if (present) {
    if (this._presencedelay) {
      clearTimeout(this._presencedelay);
      this._presencedelay = null;
      return;
    }
    ...
    this._presencedelay = setTimeout(() => {
