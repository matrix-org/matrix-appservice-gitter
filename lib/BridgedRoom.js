...
BridgedRoom.prototype.joinAndStart = function() {
    ...
    // messages we sent
    return this._main.getMyGitterUserId().then((gitterUserId) => {
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    return this._main.getOrCreateMatrixUser(message.user_id).then((user) => {
        var ghost = this._main.getGitterGhost(); // TODO(paul): this will become a method on 'user'
        ...
        if (msgtype == 'm.emote') {
            ...
            ghost.sendStatus(this._gitterRoomName, text);
            ...
            ghost.send(this._gitterRoomName, text);
            this._main.incCounter("sent_messages", {side: "remote"});
        }
        else {
            if (msgtype !== 'm.text') {
