...
BridgedRoom.prototype.unlinkMatrixRoom = function(matrixRoomId) {
    ...
    if (this._linkedMatrixRoomIds.length || this._portalMatrixRoomId) {
        ...
        return this.drainAndLeaveMatrixRoom(matrixRoomId);
    }

    return Promise.all([
        this.stopAndLeave(),
        this.drainAndLeaveMatrixRoom(matrixRoomId),
    ]);
};
...
BridgedRoom.prototype.drainAndLeaveMatrixRoom = function(matrixRoomId) {
    var bridge = this._bridge;

    return bridge.listGhostUsers(matrixRoomId).then((user_ids) => {
        console.log("Draining " + user_ids.length + " ghosts from " + matrixRoomId);

        return Promise.each(user_ids, (user_id) => {
            return bridge._bridge.getIntent(user_id).leave(matrixRoomId);
        });
    }).then(() => {
        return bridge.getBotIntent().leave(matrixRoomId);
    });
};
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    var my_member_event = this._bridge.getStoredEvent(
        message.room_id, "m.room.member", user_id
    );
    ...
    var from = this._bridge.mangleName(user_id, displayname);
    ...
    if (msgtype == 'm.emote') {
        ...
        var image_url = this._bridge.getUrlForMxc(message.content.url);
        // If we just paste markdown with the image URL directly into gitter,
        // clients will cope
        var text = '`' + from + '` posted an image: ' +
            '![' + message.content.body + '](' + image_url + ')';
