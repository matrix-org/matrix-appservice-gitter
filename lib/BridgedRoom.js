...
function BridgedRoom(bridge, config, gitter, matrixRoomModel, gitterRoomModel) {
  ...
  this._userStore = bridge.getUserStore();
...
BridgedRoom.prototype.onGitterMessage = function(message) {
  ...
  this._userStore.getRemoteUser(fromUser.username).then(function (remote_user) {
    ...
    if (remote_user)
      // TODO: test if the URL is different
      return;

    return this._setUserAvatar(intent, fromUser);
...
BridgedRoom.prototype._setUserAvatar = function(intent, fromUser) {
  var avatar_url = fromUser.avatarUrlMedium;
  console.log("Updating " + fromUser.username + " avatar image from " + avatar_url);
