...
var gitterRateLimiter = new RateLimiter(JOIN_DELAY_STAGGER);
...
BridgedRoom.prototype.joinAndStart = function() {
    ...
    return this._gitterStartingPromise = this._main.getMyGitterUserId().then((gitterUserId) => {
        ...
        return retry(() => {
            return gitterRateLimiter.next().then(() => {
...
BridgedRoom.prototype.syncUsers = function(opts) {
    ...
    return this._gitterRoom.users().then((gitterUsers) => {
        // Form the set of gitter usernames of every gitter-side user apart
        // from the relaybot
        var gitterUsernames = new Set(
            gitterUsers.filter((gitterUser) => gitterUser.id != myId)
                       .map((gitterUser) => gitterUser.username)
        );
        console.log("Found some users", gitterUsernames);
        ...
        function syncOneRoom(matrixRoomId) {
            return main.listAllUsers(matrixRoomId).then((matrixUserIds) => {
                ...
                console.log("Comparing to ", ghostMap);
        ...
        return Promise.all(this.getAllMatrixRoomIds().map(syncOneRoom));
    }).then(() => counts);
};
