...
BridgedRoom.prototype.onGitterMessage = function(message) {
    ...
    // Don't send out messages from the bridge user.
    if (this._gitter.currentUser().username.toLowerCase() === fromUser.username.toLowerCase() ) {
        log.debug("Ignoring message from the gitter bot");
