...
BridgedRoom.prototype.unlinkMatrixRoom = function(matrixRoomId) {
    ...
    if (this._linkedMatrixRoomIds.length || this._portalMatrixRoomId) {
        ...
        return this._bridge.drainAndLeaveMatrixRoom(matrixRoomId);
    }

    return Promise.all([
        this.stopAndLeave(),
        this._bridge.drainAndLeaveMatrixRoom(matrixRoomId),
    ]);
};
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    var avatar_url = (my_member_event && my_member_event.content) ?
        my_member_event.content.avatar_url : null;
    if (avatar_url) {
        var image_url = this._bridge.getUrlForMxc(avatar_url, {thumb: true});

        from = "![avatar](" + image_url + ") " + from;
    }
