...
BridgedRoom.prototype.handleGitterCommand = async function(fromUser, message) {
    ...
    if (!this._gitterAdminList.has(fromUser.username)) {
    ...
    const roomIds = new Array(this._linkedMatrixRoomIds);
    if (this._portalMatrixRoomId) {
        roomIds.push(this._portalMatrixRoomId);
    }
    ...
    if ((command === "kick" || command === "ban") && commandArgs.length === 0) {
        ...
        let userId;
        const isUserId = IS_USERID.match(givenUser);
        if (isUserId) {
            userId = givenUser;
        ...
        for (const roomId of roomIds) {
            ...
            if (!isUserId) {
                ...
                log.debug(`Matching '${givenUser}' as user id`);
            } else if (joinedMembers[userId]) {
                log.debug(`${userId} was not found in ${roomId}`);
            }

            try {
                await this._bridge.getBotIntent()[command](roomId, userId, `Requested by ${fromUser.username}`);
        ...
        response = actioned ? `Successfully completed ${command} against user` : "User could not be removed";
    ...
    function more() {
        return limiter.next().then(() => {
            ...
            batch.forEach((u) => {
                ...
                if (u.role === "admin") {
                    this._gitterAdminList.add(u.username);
                }
            });
