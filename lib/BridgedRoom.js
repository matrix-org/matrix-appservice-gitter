...
BridgedRoom.prototype.handleGitterCommand = async function(fromUser, message) {
    ...
    if ((command === "kick" || command === "ban") && commandArgs.length === 0) {
        ...
        for (const roomId of roomIds) {
            ...
            if (!isUserId) {
                // joined_members surprisngly returns per room profiles, then user profiles in that order
        ...
        response = actioned ? "Successfully completed action against user" : "User could not be removed";
