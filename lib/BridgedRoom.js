...
BridgedRoom.prototype.unlinkMatrixRoom = function(matrixRoomModel) {
    this._matrixRoomModels = this._matrixRoomModels.filter((r) =>
        r.getId() != matrixRoomModel.getId()
    );

    if (this._matrixRoomModels.length) return Promise.resolve();
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    this._matrixRoomModels.forEach((room) => {
        if (room.getId() == message.room_id) return;

        this._bridge.getBotIntent().sendMessage(
            room.getId(), {
                msgtype: "m.text",
                body: "`"+message.sender+"` "+message.content.body,

                format: "org.matrix.custom.html",
                formatted_body: "<code>"+message.sender+"</code> "+message.content.body,
            }
        );
    });
};
