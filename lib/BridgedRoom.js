...
// How old the gitter member list must be before we mark it as stale.
const STALE_USER_LIST = 3600 * 1000; // 1 hour
...
function BridgedRoom(opts) {
    ...
    this._gitterAdminList = null; // Set(username)
    this._gitterUsersLastFetch = null;
    this._gitterUsers = null;
}
...
BridgedRoom.prototype.swapRoomIds = function(oldRoomId, newRoomId) {
    if (this._portalMatrixRoomId === oldRoomId) {
        this.setPortalMatrixRoomId(newRoomId);
    }

    const index = this._linkedMatrixRoomIds.indexOf(oldRoomId);
    if (index) {
        this._linkedMatrixRoomIds.splice(index, 1);
        this.linkMatrixRoom(newRoomId);
    }
}
...
BridgedRoom.prototype.onGitterMessage = async function(message) {
    // Each message follows the model given in
    //   https://developer.gitter.im/docs/messages-resource
    const botIdentity = await this._main.getRelayGhost().identity();

    var fromUser = message.fromUser;

    var user_id = fromUser.id;

    if (user_id === botIdentity.id) {
        // Never bridge across bot messages
        return;
    }
    ...
    return this._main.getGitterUser(user_id, {
        ...
        if (message.text.startsWith("!matrix")) {
            return this.handleGitterCommand(user, message.text);
        }
...
BridgedRoom.prototype.handleGitterCommand = async function(fromUser, message) {
    const IS_USERID = /^@.+:.+$/; // Lazy user id regex.
    if (message === "!matrix") {
        message = "!matrix help";
    }

    // Determine if the user is an admin.
    await this._listGitterUsers();

    if (!this._gitterAdminList.has(fromUser._gitter_id)) {
        await this._main.getRelayGhost().send(this._gitterRoomName, "Only admins may use the !matrix command");
        return;
    }


    const commandParts = message.substr("!matrix ".length).split(" ");
    const command = commandParts[0].toLowerCase().trim();
    const commandArgs = commandParts.slice(1);

    const roomIds = this.getAllMatrixRoomIds();

    let response = "Command not understood";
    if ((command === "kick" || command === "ban") && commandArgs.length === 0) {
        response = `\`${command}\` needs a name parameter. Specify a matrix user ID (@username:domain) or displayname`;
    } else if (command === "kick" || command === "ban") {
        const givenUser = commandArgs.join(" ");
        let userId;
        const isUserId = IS_USERID.exec(givenUser) !== null;
        if (isUserId) {
            userId = givenUser;
            log.debug(`Matching '${givenUser}' as user id`);
        }
        // Because we can't ask the gitter admin to target a single room, we run this across
        // all rooms. If we manage to do it once, consider it a success.
        let actioned = false;
        let foundUser = false;
        for (const roomId of roomIds) {
            let joinedMembers;
            try {
                joinedMembers = await this._main.getJoinedMembers(roomId);
            } catch (ex) {
                log.warn(`Request to joined_members failed in ${roomId}. Ignoring room.`);
                log.debug("Exception:", ex);
                continue;
            }

            // This must be run for each room.
            if (!isUserId) {
                // joined_members surprisingly returns per room profiles, then user profiles in that order
                // which means we don't need to lookup user's membership info \o/
                userId = Object.keys(joinedMembers).find((u) => joinedMembers[u].display_name === givenUser);
                if (!userId) {
                    log.info(`Failed to find ${givenUser} in ${roomId}`);
                    continue;
                }
                log.debug(`Matching '${givenUser}' as user id`);
            }

            if (!joinedMembers[userId]) {
                log.debug(`${userId} was not found in ${roomId}`);
                continue;
            } else {
                foundUser = true;
            }
            ...
            try {
                await this._main.getBotIntent()[command](roomId, userId, `Requested by ${fromUser.gitterUsername()}`);
                log.info(`Actioned ${command} against ${userId} in ${roomId}`);
                actioned = true;
            } catch (ex) {
                // TODO: Report these errors to Gitter?
                log.debug(`Failed to ${command} ${userId} in ${roomId}:`, ex);
            }
        }

        if (actioned) {
            return;
            // Don't say anything because we echo kicks and bans to gitter.
        } else if (!foundUser) {
            response = "No users matched that name";
        } else {
            response = "User could not be removed";
        }
    } else if (command === "help") {
        response = "!matrix kick|ban user_id|display_name";
    }
    if (response) {
        await this._main.getRelayGhost().send(this._gitterRoomName, response);
    }
}
...
BridgedRoom.prototype.onMatrixMessage = function(message) {
    ...
    return Promise.all([
        ...
        const guard_p = send_p.catch((e) => {
            log.warn(`Failed to send message to ${this.gitterRoomName()} from ${from}: ${e}`);
        });
...
BridgedRoom.prototype._listGitterUsers = async function() {
    ...
    if (this._gitterUsers && (Date.now() - this._gitterUsersLastFetch) < STALE_USER_LIST) {
        return;
    }

    this._gitterAdminList = new Set();
    ...
    const more = () => {
        return limiter.next().then(() => {
            ...
            batch.forEach((u) => {
                ...
                if (u.role === "admin") {
                    this._gitterAdminList.add(u.id);
                }
            });

            // We got to the end
            if (batch.length < 100) {
                this._gitterUsersLastFetch = Date.now();
                this._gitterUsers = names;
                return;
            }
