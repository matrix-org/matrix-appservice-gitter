...
function Main(config) {
    ...
    // map access tokens to GitterGhost instances
...
Main.prototype.initialiseMetrics = function() {
    ...
    this._bridge.registerBridgeGauges(() => {
        ...
        return {
            ...
            remoteGhosts:
                Object.keys(this._gitterGhostsByMatrixId).length,
...
Main.prototype.getRelayBotGitterGhost = function() {
    return this._relayBotGitterGhost;
};

Main.prototype.createGitterGhost = function(access_token) {
    var ghost = new GitterGhost({
        main: this,
        gitter: new Gitter(access_token),
        is_puppet: true,
        rateLimiter: new RateLimiter(GITTER_LIMITER_INTERVAL),
    });

    return this._gitterGhostsByMatrixId[access_token] = ghost;
};
...
Main.prototype.putUser = function(user) {
    ...
    return this._bridge.getUserStore().upsert({type: entry.type, id: entry.id}, entry);
};
...
Main.prototype.makeAuthorizeURLForUser = function(opts) {
    var user = opts.user;

    var oauth2 = this._oauth2;
    if (!oauth2) {
        return Promise.reject("OAuth2 is not configured");
    }

    // TODO: optionally take 'user_id' instead and call getOrCreateMatrixUser

    return user.generateRandomKey().then((randkey) => {
        var state = [randkey, user.userId()].join("|");
        return oauth2.makeAuthorizeURL({state: state});
    });
};

Main.prototype.handleIncomingAuthorize = function(opts) {
    // TODO: need to work out exactly what sort of values this would receive in a real case
    ...
    var _statesplit = state.split("|");
    var randkey = _statesplit[0];
    var user_id = _statesplit[1];

    return this.getOrCreateMatrixUser(user_id).then((user) => {
        return user.checkRandomKey(randkey).then(() => {
            return oauth2.exchangeCodeForToken({code: opts.code});
        }).then((response) => {
            return user.setGitterAccessToken(response.access_token);
        });
    });
};

Main.prototype.addOAuth2AppServicePath = function(bridge) {
    bridge.addAppServicePath({
        method: "GET",
        path: "/oauth2/grant",
        handler: (req, res) => {
            this.handleIncomingAuthorize(req.query).then(
                () => { // success
                    res.writeHead(200, {"Content-Type": "text/html"});
                    res.write(`
<h2>Integration Successful</h2>

<p>Your Matrix-Gitter user integration is now correctly authorized.</p>
`);
                    res.end();
                },
                (e) => { // failure
...
<p>Unfortunately your user integration did not go as expected...</p>
`);
                    res.end();
                }
            );
...
Main.prototype.run = function(port) {
    ...
    this.addOAuth2AppServicePath(bridge);
