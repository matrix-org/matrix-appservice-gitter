...
const { Bridge, StateLookup, PrometheusMetrics } = require("matrix-appservice-bridge");
...
const log = require("./Logging.js").Get("Main");
...
function Main(config, registration) {
    ...
    const roomStore = path.join(config.data_dir || "", "room-store.db");
    const userStore = path.join(config.data_dir || "", "user-store.db");
    ...
    const bridge = new Bridge({
        ...
        userStore,
        roomUpgradeOpts: {
            migrateEntry: (entry, newId) => {
                const oldId = entry.matrix.roomId;
                log.info(`Migrating room ${oldId} -> ${newId}`);
                try {
                    entry.matrix.roomId = newId;
                    entry.id = entry.id.replace(oldId, newId);
    
                    const room = this._bridgedRoomsByMatrixId[oldId];
                    if (!room) {
                        // We don't have a room for this one yet?
                        return entry;
                    }
                    room.swapRoomIds(oldId, newId);
                    delete this._bridgedRoomsByMatrixId[oldId];
                    this._bridgedRoomsByMatrixId[newId] = room;
                } catch (ex) {
                    log.error("Failed to handle room upgrade:", ex);
                }
                return entry;
            }
        },
        controller: {
            ...
            onEvent: (req, context) => {
                ...
                const bridgeBot = bridge.getBot();

                const isRemoteUser = bridgeBot.isRemoteUser(event.sender);

                const endTimer = this.startTimer("matrix_request_seconds");

                this._stateStorage.onEvent(event);

                if (event.type == "m.room.member" && (!event.state_key || event.state_key === "")) {
                    // Be careful not to handle non-state events.
                    return;
                }

                if (event.type == "m.room.member" &&
                        event.state_key == bridgeBot.getUserId()) {
                    const membership = event.content.membership;
                    if (membership == "invite") {
                        this.onBotInvited(event.room_id);
                    }
                    else if (membership == "join") {
                        this.onBotJoined(event.room_id);
                    }
                    else if (membership == "leave") {
                        // TODO: "leave" events might mean we left the room, or got kicked
                        //   while still in invite state - i.e. invite was cancelled
                        //   before we joined.
                        this.onBotLeft(event.room_id);
                    }
                    else {
                        log.debug('matrix member event for myself to state ' + membership +
                                ' from state ', event);
                    }
                    endTimer({outcome: "success"});
                    return;
                } else if (event.type == "m.room.member" && 
                    bridge.getBot()._isRemoteUser(event.state_key) &&
                    event.sender !== bridge.getBot().getUserId()) {

                    // We got an invite but the bot doesn't support them, warn about it.
                    log.info(`${event.state_key} got invite for ${event.room_id} but we can't do DMs, warning room.`);
                    ...
                    intent.join(event.room_id).then(() => {
                        return intent.sendEvent(event.room_id, "m.room.message", {
                            msgtype: "m.notice",
                            body: "The gitter bridge doesn't support private messaging, or inviting to rooms.",
                        });
                    }).then(() => {
                ...
                if (event.type === "m.room.member" && event.sender !== event.state_key && !isRemoteUser) {
                    this.handleMatrixKickBan(event).then(() => {
                        log.info(`Handled kick/ban for ${event.state_key} (by ${event.sender})`);
                        endTimer({outcome: "success"});
                    }).catch((ex) => {
                        log.warn(`Failed to handle kick/ban for ${event.state_key} (by ${event.sender}):`, ex);
                ...
                log.debug('matrix->' + event.room_id + ' from ' + event.user_id);
                ...
                if (!handled) {
                    log.info("  Wasn't expecting this room; ignore");
                    endTimer({outcome: "dropped"});
...
Main.prototype.handleMatrixKickBan = async function(event) {
    const room = this.getBridgedRoomByMatrixId(event.room_id);
    if (!room) {
        return;
    }
    const membership = event.content.membership;
    const bridgeBot = this._bridge.getBot();
    let keyword;
    ...
    if (membership === "leave") {
        ...
        return; // Don't mention joins or invites, too noisy.
    }

    let targetName = event.state_key;
    if (event.prev_content) {
        targetName = event.prev_content.displayname || targetName;
    }

    let senderName = event.sender;
    let reason = event.content.reason ? ` ("${event.content.reason}")` : "";

    if (event.sender === bridgeBot.getUserId() && reason && event.content.reason.startsWith("Requested by ")) {
        // Lazy extraction of real sender
        senderName = event.content.reason.substr("Requested by ".length);
        reason = "";
    } else if (!bridgeBot.isRemoteUser(event.sender)) {
        const user = await this.getOrCreateMatrixUser(event.sender);
        senderName = user.getDisplaynameForRoom(event.room_id) || event.sender;
    }

    const message = `\`${targetName}\` was **${keyword}** by \`${senderName}\`${reason}.`;
    await this._relayBotGitterGhost.send(room.gitterRoomName(), message);
}
...
Main.prototype.getUrlForMxc = function(mxc_url) {
    return path.join(this.media_url, "_matrix/media/v1/download", mxc_url.substring("mxc://".length));
};
...
Main.prototype.getRelayGhost = function() {
...
Main.prototype.getGitterUser = function(id, opts) {
    ...
    return this._bridge.getUserStore().select({id: id}).then((entries) => {
        ...
        if (entries.length) {
            ...
            log.info("Cleaning up legacy user account " + mxid_localpart);
...
Main.prototype.getJoinedMembers = function (roomId) {
    return this._bridge.getBot().getJoinedMembers(roomId);
}
...
Main.prototype.getOrCreateBridgedRoom = function(gitterName) {
    ...
    var bridgedRoom = new BridgedRoom({
        ...
        enablePresence: this._config.enable_presence,
    });
...
Main.prototype.drainAndLeaveMatrixRoom = function(matrixRoomId) {
    return this.listGhostUsers(matrixRoomId).then((user_ids) => {
        log.info("Draining " + user_ids.length + " ghosts from " + matrixRoomId);
        ...
        instances: [
            {
                ...
                desc: "Gitter",
                icon: this._config.icon_uri, // in case clients don't respect the toplevel one
                fields: {},
            },
        ],
    });
...
Main.prototype.makeMatrixRoom = function(opts) {
    ...
    return intent.createRoom({
        ...
        // Associate the room with this application service, so room directory
        // filtering can work properly
        return intent.getClient().setRoomDirectoryVisibilityAppService(
            NETWORK_ID, room_id, "public"
        ).then(() => room_id);
    });
};
    ...
    bridge.loadDatabases().then(() => {
        ...
        entries.forEach((entry) => {
            ...
            .then((room) => {
                ...
                log.warn("Failed to link " + matrix_id + " to " + remote_id, e);
            });
        });
    });
...
Main.prototype.actionLink = function(matrixId, gitterName) {
    ...
    return store.getEntriesByMatrixId(matrixId).then((remoteLinks) => {
        ...
        log.info("LINKED " + matrixId + " to " + gitterName);
    });
};
...
Main.prototype.actionUnlink = function(matrixId, gitterName) {
    ...
    return store.delete({id: linkId}).then(() => {
        ...
        log.info("UNLINKED " + matrixId + " to " + gitterName);
    });
};

Main.prototype.actionMakePortal = function(gitterName) {
    log.debug("actionMakePortal:", gitterName);
    var store = this.getRoomStore();
...
Provisioning.commands.link = new Provisioning.Command({
    ...
    func: function(main, req, res, matrixId, gitterName, userId) {
        log.info(`Got link request: ${userId} is trying to link ${matrixId} to ${gitterName}`);
...
Provisioning.commands.unlink = new Provisioning.Command({
    params: ["matrix_room_id", "remote_room_name", "user_id"],
    func: function(main, req, res, matrixId, gitterName, userId) {
        log.info(`Got unlink request: ${userId} is trying to unlink ${matrixId} from ${gitterName}`);
        return main.checkLinkPermission(matrixId, userId).then((allowed) => {
            if (!allowed) return Promise.reject({
                code: 403,
                text: userId + " is not allowed to provision links in " + matrixId,
