...
Main.prototype.handleMatrixKickBan = async function(event) {
    ...
    if (!bridgeBot.isRemoteUser(event.state_key)) {
        const user = await this.getOrCreateMatrixUser(event.state_key);
        targetName = user.getDisplaynameForRoom(event.room_id) || event.state_key;
    }
    ...
    // This is a kick for one of our users
}
