...
MatrixUser.prototype.getGitterGhost = function() {
    ...
    // TODO(paul): do some sort of 'entity' storage locally
    return main._bridge.getUserStore().select({
        type: "matrix",
        id: this._user_id,
    }).then((entries) => {
        if (!entries.length) {
            return this._gitterGhost = main.getRelayBotGitterGhost();
        }
        ...
        return this._gitterGhost = main.createGitterGhost(entry.data.gitter_access_token);
    });
};
