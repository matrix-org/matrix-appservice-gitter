...
MatrixUser.prototype.getGitterGhostForRoom = function(room_name) {
    var main = this._main;

    if (!this._gitterAccessToken) {
        // No token - we'll not be puppeting then
        return Promise.resolve(main.getRelayBotGitterGhost());
    }

    var ghost = this._gitterGhost = this._gitterGhost ||
        main.createGitterGhost(this._gitterAccessToken)

    // TODO: only return the ghost if it's really in the room
