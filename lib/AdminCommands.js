...
adminCommands.list = new AdminCommand({
    ...
    opts: {
        'failed|F': "list only failed rooms",
    },
    func: function(main, opts, args, respond) {
        Object.keys(main._bridgedRoomsByGitterRoomname).sort().forEach((gitterName) => {
            ...
            if (opts.failed && status !== "FAILED") return;
...
adminCommands.set_access_token = new AdminCommand({
    ...
    func: function(main, args, respond) {
        ...
        return main.getOrCreateMatrixUser(user_id).then((u) => {
            return u.setGitterAccessToken(access_token);
        }).then(() => {
            // Remove the cached ghost so it gets recreated if required
...
adminCommands.oauth2 = new AdminCommand({
    desc: "begin the OAuth2 flow for a user",
    func: function(main, args, respond) {
        var user_id = args.shift();

        return main.getOrCreateMatrixUser(user_id).then((user) => {
            return main.makeAuthorizeURLForUser({user: user});
        }).then((url) => {
            respond("URL: " + url);
        });
    }
...
adminCommands.oauth2_inject = new AdminCommand({
    desc: "inject an incoming OAuth2 redirect URL",
    func: function(main, args, respond) {
        var url = args.shift();

        // Trim just the query string
        var idx = url.indexOf("?");
        if (idx > -1) {
        ...
        return main.handleIncomingAuthorize(params).then(() => {
            respond("OAuth2 successful");
        });
    }
});
