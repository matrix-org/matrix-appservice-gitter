...
adminCommands.link = new AdminCommand({
    ...
    func: function(bridge, args, respond) {
        ...
        return bridge.actionLink(matrixId, gitterName).then(
            ()    => { respond("Linked"); },
...
adminCommands.leave = new AdminCommand({
    ...
    func: function(bridge, args, respond) {
        ...
        if (bridge.getBridgedRoomByMatrixId(matrixId)) {
        ...
        return bridge.listGhostUsers(matrixId).then((user_ids) => {
            respond("Draining " + user_ids.length + " ghosts from " + matrixId);

            return Promise.each(user_ids, (user_id) => {
                return bridge._bridge.getIntent(user_id).leave(matrixId);
            });
        }).then(() => {
            return bridge.getBotIntent().leave(matrixId);
        }).then(() => {
