...
adminCommands.cleanup_lowercase = new AdminCommand({
    desc: "clean up lowercased legacy users",
    func: function(main, args, respond) {
        ...
        return store.select({
            ...
            respond("Found " + entries.length + " user entries...");

            var count = 0;

            return Promise.each(entries, (entry) => {
                count++;
                if (count % 100 == 0) { console.log("  done " + count + "..."); }

                var username = entry.data.username;
                if (username !== username.toLowerCase()) {
                    return;
                }

                var mxid_localpart = main.localpartFromGitterName(username);

                var id = entry.id;
                var u = main._gitterUsersById[id];
                if (u) {
                    u._mxid_localpart = mxid_localpart;
                }

                entry.data.mxid_localpart = mxid_localpart;
                return store.upsert({id: entry.id}, entry);
            });
        });
    },
});
