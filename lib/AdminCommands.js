...
function resolveBridgedRoom(main, id) {
    return resolveRoomEntries(main, id).then((entries) => {
        ...
        var entry = entries[0];
        return main.getBridgedRoomByGitterName(entry.remote_id);
    });
}
...
adminCommands.sync_users = new AdminCommand({
    ...
    func: function(main, opts, args, respond) {
        ...
        return resolveBridgedRoom(main, id).then((room) => {
            return room.syncUsers({
                join: opts.join,
                leave: opts.leave,
                countOnly: opts.count,
                rateLimiter: new RateLimiter(2*1000), // msec
            })
        }).then((counts) => {
            respond("Join " + counts.join + " and leave " + counts.leave + " users");
        });
    },
});
